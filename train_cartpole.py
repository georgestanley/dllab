...
from utils import EpisodeStats,action_to_id,id_to_action
...
def train_online(env, agent, num_episodes, model_dir="./models_cartpole", tensorboard_dir="./tensorboard"):
    ...
    tensorboard = Evaluation(os.path.join(tensorboard_dir, "train"),  name="Training Cartpole", stats=["episode_reward", "a_0", "a_1"])
    ...
    for i in range(num_episodes):
        ...
        ep_rewards.append(stats.episode_reward)
        ...
        if i% eval_cycle == 0:
            ...
            print("mean episode reward=",reward)
            evals.append(reward)

        if i % eval_cycle == 0 or i >= (num_episodes - 1):
            agent.save(os.path.join(model_dir, "episode_"+str(i)+"_dqn_agent.pt"))
...
if __name__ == "__main__":
    ...
    net = MLP(state_dim,num_actions)
    dqn_agent = DQNAgent(net, net,num_actions , history_length=1e6)
    train_online(env,agent=dqn_agent,num_episodes=200 )

    plt.plot(evals, label="Episode Rewards")
    plt.xlabel("Minibatches")
    plt.ylabel('Reward')
    plt.legend()
    plt.show()

    plt.plot(ep_rewards, label="Mean Reward")
    plt.xlabel("Minibatches")
    plt.ylabel('Reward')
    plt.legend()
    plt.show()
